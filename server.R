...
shinyServer(function(input, output, session) {
  ...
  timeSeries_DateRange_Std <- reactive({
    ...
    dx <- switch(
      input$timeSeries_DateRange_Std,
  ...
  volatility_DateRange_Std <- reactive({
    ...
    lx <- tail(ix, 1) # last date
    dx <- switch(
      input$volatility_DateRange_Std,
      "1" = range(tail(ix, 5)),                      # 5D
      "2" = c(as.Date(lx-dyears(1/12)), lx),         # 1M
      "3" = c(as.Date(lx-dyears(0.5)), lx),          # 6M
      "4" = c(as.Date(lx-dyears(1)), lx),            # 1Y
  ...
  usedSeries <- reactive({
    series <- valSeries()
    dr <- timeSeries_DateRange() %>% format() %>% paste(collapse="/")
    series[dr]
  })
  ...
  retSeries <- reactive({
    PerformanceAnalytics::Return.calculate(valSeries(), method = "log") %>% na.trim
  })
  
  volatility_retSeries <- reactive({
    dr <- volatility_DateRange() %>% format() %>% paste(collapse="/")
    retSeries()[dr]
  })
  
  garchFit <- reactive({
    ret <- volatility_retSeries()
    fGarch::garchFit(data=zoo::coredata(ret), include.mean=FALSE, trace=FALSE)
  })
  ...
  output$returns <- renderTable({
    ...
    data.frame(Date=as.character(index(ret)),
  ...
  output$volatility_summary <- renderUI({
    ...
    div(
      ...
      p("Long term vol. = ", format(longTermVolatility() * 100, digits = 1, nsmall = 2), "%",
  ...
  output$timeSeries <- renderDygraph({
    ...
    series <- merge.xts(returns = ret, top = vol * qnorm(ic), bottom = -vol * qnorm(ic), violation = viol) * 100
  ...
  output$price <- renderValueBox({
    series <- usedSeries()
    last_price <- series |> tail(1) |> as.numeric()
  ...
  output$period_return <- renderValueBox({
    series <- usedSeries()
    ret <- log(series) |> diff() |> na.omit() |> sum() |> as.numeric()
  ...
  output$return_1m <- renderValueBox({
    x <- valSeries() |> last(21) |> log() |> diff() |> na.omit() |> sum() |>
      as.numeric()
  ...
  output$return_1y <- renderValueBox({
    x <- valSeries() |> last(252) |> log() |> diff() |> na.omit() |> sum() |>
      as.numeric()
